        ...
        <tbody>
         ...
         @if(isset($getAllRequest) && count($getAllRequest) >0)
          ...
          <tr>
           ...
           <td>{{$key->RemarkHo}}</td>  
            <td>{{$key->AmountHo}}</td>  
           <td>@if($key->status=='1'){{'Pending'}} @else {{'Approved'}}  @endif</td> 
           <td>@if($key->statusByHo=='3'){{'Pending'}} @else {{'Approved'}}  @endif</td>  
           @if(Session::get("id")->Last_Name=='6')

         	<td>
                 @if($key->status!='1' && $key->statusByHo!='3')
                    <button style="cursor:no-drop;" type="button" id="a{{$i}}" class="btn btn-success btnSubmit" >Approved</button>
                 @else
                 <button type="button" id="a{{$i}}" class="btn btn-primary btnSubmit" @if($key->status!='1') @if($key->statusByHo=='3') onclick="modelopen('{{$key->id}}');" @endif @else onclick="alert('Not Approved By Manager');" @endif >Approve</button>
                
                @endif

            </td>
         	@elseif(Session::get("id")->Last_Name!='6')
         	<td>
                    @if($key->status=='1')
                    <button type="button" id="a{{$i}}" class="btn btn-primary btnSubmit"  onclick="Approved('{{$key->id}}');" >Approve</button>
                    @else
                     <button style="cursor:no-drop;" type="button" id="a{{$i}}" class="btn btn-success btnSubmit"   >Approved</button>
                    @endif
            </td>
...
<div id="DiscountModel"></div>
  ...
  <script type="text/javascript">
     ...
     function Approved(depoId,RemHo=null,AmHo=null,pageReq=null){
		       ...
		       url: base_url + '/webadmin/PostCashRequestApproved',
		       cache: false,
		       data: {
